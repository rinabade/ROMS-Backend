    ...
    actions: {
        create: {
            ...
            //     job_title: {type: "string", max: 10},
                // salary_information : {type: "string", max: 10},
                // employee_status : {type: "string", max: 10},
            // },
            async handler(ctx) {
                const { firstname, lastname, email, password, gender, address, phone, job_title, salary_information, employee_status } = ctx.params;
                // Get the current date and time
                const now = moment();
                // Format the date and time
                const formattedNow = now.format('YYYY-MM-DD HH:mm:ss');

                    if (!firstname || !lastname || !email || !password || !gender || !address || !phone || !job_title || !salary_information || !employee_status) {
                        ...
                        if (rows[0]) {
                            ...
                            const [result] = await connection.execute(`SELECT role_id FROM roles WHERE role_name = ?`, [role]);
                            const role_id = result[0].role_id;

                            const Npassword = await bcrypt.hash(password, 8);
                            const [result1] = await connection.execute('INSERT INTO employees(role_id, firstname, lastname, email, password, gender, address, phone, job_title, salary_information, employee_status, createdAt) VALUES (?,?,?,?,?,?,?,?,?)', [role_id,firstname, lastname, email, Npassword, gender, address, phone, job_title, salary_information, employee_status, formattedNow]);
                            if (result1)
